...
which uses Docker as its primary deployment method.
...
Initially, this approach worked well enough, but I soon realized that the Docker image for the web dashboard had grown to an enormous **2.19GB**!
...
My "this will do for now" mindset lead me to adding `pnpm drizzle migrate` to my start script and adding a step to install _all_ dependencies in the `RUNNER` stage of the Dockerfile...
...
Removing all the database migration related steps, here's what the `RUNNER` stage looks like:
...
By default, Next.js generates a `.next` folder with all the necessary files for your app but doesn't account for any dependencies used.
...
Rebuilding the Docker image again and the size dropped from **256.1MB** to a puny **187.4MB**!

Going from **2.19GB** to **187.4MB** is a massive improvement, and I couldn't be happier with the results. However, we don't have database migrations yet.
...
Now that migrations are ran during runtime, I can remove all the steps related to installing dependencies during the `RUNNER` stage.
...
Rebuilding the image one last time reveals the the size of the Docker image has increased slightly to **187.4MB**.
...
If you have any questions or suggestions, feel free to reach out to me on [BlueSky](https://bsky.app/profile/tygr.dev) or join my [Discord server](/discord).
